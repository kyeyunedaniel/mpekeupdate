@extends('admin.app')
@section('title') Transporter @endsection
@section('content')
    <div class="app-title">
        <div>
            <h1><i class=""></i> Transporter</h1>
        </div>
                        ...
                        <tbody>
                            @foreach($ware_house as $employ)
                                ...
                                <td>{{ $employ->name }}</td>
                                    ...
                                    <td class="text-right">
										<div class="actions">
										<a class="btn btn-sm bg-success-light text-info" data-toggle="modal" href="{{route('admin.Trasporter.edit',$employ->id)}}" data-target="#edit_specialities_details{{$employ->id}}">
											<i class="fe fe-pencil"></i> Edit
										</a>

											<a  data-toggle="modal" href="#delete_modal{{$employ->id}}" class="btn btn-sm bg-danger-light">
							...
							<form action="{{route('admin.Trasporter.edit')}} " method="post">
								@csrf
								   <div class="form-group card-label">
			                        ...
			                        <div class="form-group card-label">
			                            <label for="Description" class="col-form-label"><strong>Username</strong> </label>
			                            <textarea class="form-control @error('CompanyDescription') is-invalid @enderror" name="action"  rows="1" cols="2" required>{{old('action', $try_m->username)}}</textarea>
			                        </div>
			                        <div class="form-group card-label">
			                            <label for="recipient-name" class="col-form-label"><strong>Email</strong> </label>
			                            <input value="{{old('logo', $try_m->email)}}"  type="text" class="form-control" id="name" name="email"
			                                  >
			                        </div>
								<button type="submit" class="btn btn-primary btn-block">Save Changes</button>
							</form>
